...
const Features = () => {
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (scrollContainerRef.current) {
      const scrollAmount = scrollContainerRef.current.clientWidth * 0.8; // Scroll sejauh 80% lebar layar
  ...
  useEffect(() => {
    ...
    // Event listener untuk update tombol panah saat di-scroll
  ...
  return (
    <section className="py-20 px-0 sm:px-6 md:px-12 bg-emerald-50 dark:bg-slate-950 transition-colors duration-500 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative group/slider">
        ...
        {/* Container Grid (Laptop) / Flex Scroll (HP) */}
        {/* Di HP: flex-row overflow-x-auto, Di Laptop: grid grid-cols-4 */}
        <div 
          ref={scrollContainerRef}
          className="flex sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 overflow-x-auto sm:overflow-visible snap-x snap-mandatory hide-scrollbar px-6 sm:px-0 pb-8 sm:pb-0"
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} // Sembunyikan scrollbar bawaan
        >
          {features.map((item, index) => (
            <div 
              key={index} 
              // Animasi beruntun (delay berbeda tiap kotak)
              className="scroll-anim-features opacity-0 translate-y-24 transition-all duration-1000 ease-out flex-shrink-0 w-[85vw] sm:w-auto snap-center"
              style={{ transitionDelay: `${index * 150}ms` }}
            >
              <div className="relative bg-white dark:bg-gray-900 p-8 rounded-[2rem] shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 border border-emerald-100 dark:border-gray-800 flex flex-col h-full group">
      ...
      <div className="mt-6 sm:mt-16 text-center scroll-anim-features opacity-0 translate-y-24 transition-all duration-1000 delay-[600ms] ease-out px-6">
        <Link 
          href="/program"
