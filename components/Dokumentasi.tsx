...
import { Camera, Loader2, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Dokumentasi = () => {
  ...
  useEffect(() => {
    const fetchDokumentasi = async () => {
      setLoading(true);
      // Panggil maksimal 5 data dari database untuk galeri
  ...
  useEffect(() => {
    ...
    const hiddenElements = document.querySelectorAll('.scroll-anim-dokumentasi');
  ...
  return (
    <div className="w-full mt-24 mb-16 relative">
      {/* HEADER SECTION */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12 border-b border-gray-100 dark:border-gray-800 pb-8 px-0 scroll-anim-dokumentasi opacity-0 translate-y-24 transition-all duration-1000 ease-out">
        <div className="max-w-xl">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold text-emerald-950 dark:text-white leading-tight tracking-tight">
            Dokumentasi <span className="text-yellow-500">Kegiatan</span>
          </h2>
        </div>
        <p className="text-gray-500 dark:text-gray-400 text-sm md:text-base max-w-sm leading-relaxed">
          Potret berbagai aktivitas santri, kegiatan belajar mengajar, dan momen berharga di Pondok Pesantren Cendekia.
        </p>
      </div>
      ...
      {loading ? (
        <div className="py-20 flex justify-center items-center gap-3 text-gray-400">
          <Loader2 className="animate-spin" /> Menyiapkan Dokumentasi...
        </div>
        ...
        <div className="relative group/slider">
          ...
          >
            {dokumentasi.slice(0, 5).map((d, index) => (
              <div 
                key={d.id} 
                className={`scroll-anim-dokumentasi flex opacity-0 translate-y-24 transition-all duration-1000 ease-out group relative flex-shrink-0 w-[85vw] md:w-auto snap-center ${getFigmaBentoClass(index)}`}
              ...
              >
                <div className="relative w-full rounded-[2rem] overflow-hidden bg-gray-100 dark:bg-gray-800 h-[350px] md:h-full">
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-center items-center text-center opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                    <h3 className="text-xl md:text-2xl font-bold text-white mb-2 shadow-sm">
                      {d.judul}
                    </h3>
                    {index === 0 && d.deskripsi && (
        ...
        <div className="p-16 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-[2rem] text-center text-gray-400">
           Belum ada galeri dokumentasi.
        </div>
      )}

      {/* TOMBOL LIHAT SEMUA */}
      <div className="mt-8 md:mt-16 text-center scroll-anim-dokumentasi opacity-0 translate-y-24 transition-all ease-out px-6">
          <Link href="/tentang#dokumentasi" className="group inline-flex items-center justify-center gap-3 bg-yellow-500 text-emerald-950 px-8 py-4 rounded-xl font-bold hover:bg-yellow-400 hover:scale-105 transition-all duration-300 shadow-xl shadow-yellow-500/30 active:scale-95 text-base md:text-lg w-full md:w-auto whitespace-nowrap">
              Cek Selengkapnya
