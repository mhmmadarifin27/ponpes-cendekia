...
const Navbar = () => {
  ...
  // Efek untuk mendeteksi scroll agar navbar bisa berubah sedikit saat discroll
  useEffect(() => {
    setMounted(true);
  ...
  const handleSearch = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && searchQuery.trim() !== '') {
      router.push(`/warta?q=${encodeURIComponent(searchQuery.trim())}`);
      setSearchQuery('');
    }
  };
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 flex flex-col w-full transition-all duration-300">
      ...
      <nav className={`
        relative z-30 px-6 md:px-12 py-4 transition-all duration-300 border-b
        ${scrolled 
